...
namespace Buddhabrot
{
	public class Brot
	{
		...
		{
			if (args.Length == 0)
				args = "-file c:\\dev\\brotCpu.bmp -w 2160 -h 3840 -it 1000 -xmin -2.0 -xmax 1.1 -samples 1000000000".Split();
			...
			double[][] arrays = null;
			var brot = new BuddhabrotCpu(width, height, xmin, xmax, iter, samples);
			...
			process.Wait();
			SaveImage(file, width, height, arrays);
